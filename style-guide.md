...
- **Streaming over buffering** - Prefer streaming patterns for object data where possible; Gmail API requires buffering for email construction but avoid unnecessary copies
...
Gmail API errors are mapped to appropriate S3 error codes:

- Gmail 404 -> `NoSuchKey` / `NoSuchBucket`
- Gmail rate limit -> `503 SlowDown`
- Other Gmail errors -> `502 InternalError`
...
**S3 API requests** produce audit entries with the `request_id`:
...
- The ID is also set as a `g3.request_id` attribute on OpenTelemetry spans
...
- Prefer generated mocks for all tests
...
- Run against real Gmail API with a test label prefix
- Cover end-to-end flows: CRUD, listing, chunking
...
- **Mock Gmail service** wraps the Gmail client to inject errors for error-path testing
