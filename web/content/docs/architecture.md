...
document.addEventListener('DOMContentLoaded', function() {
  const nodeInfo = {
    ...
    'DRIVE':      { title: 'Google Drive API', detail: 'Stores and retrieves object data as Drive files in a root folder. Uploads use the resumable protocol with 8 MB chunks for reliability on large files. No file size limit (up to 5TB). Separate API quota pool from Gmail: 12,000 requests/user/minute.' },
